...
// Simple state factoids to be imported
let stateFactoids = [
    // Alabama
...
// Save this file to a JSON format that can be imported
func saveFactoidsToJSON() {
    print("Creating factoids JSON file...")
    
    // Format each factoid with the required fields
    let formattedFactoids = stateFactoids.map { factoid -> [String: Any] in
    ...
    // Convert to JSON
    do {
        let jsonData = try JSONSerialization.data(withJSONObject: formattedFactoids, options: .prettyPrinted)
        
        // Save to user's Desktop
        let desktopURL = FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent("Desktop")
        let fileURL = desktopURL.appendingPathComponent("StateFactoids.json")
        
        try jsonData.write(to: fileURL)
        print("✅ Success! Factoids JSON saved to: \(fileURL.path)")
        print("You can use this file to import factoids into your app.")
        print("\nTo use these factoids:")
        print("1. Open your app in Xcode")
        print("2. Add this JSON file as a resource in your project")
        print("3. Modify your app to load these factoids into CloudKit when they're missing")
    } catch {
